...
def footer(prefix, language, settings):
	...
	return f'''<footer class="site-footer"><div class="container footer-grid"><div><img src="{asset(settings.get('logo'), prefix, 'images/hks-surfaces-logo.png')}" alt="HKS Surfaces" style="width:120px;border-radius:50%"></div><div><h3>{text(settings.get('site_name', 'HKS Surfaces'))}</h3><p><strong>{"Head Office" if language == "en" else "สำนักงานใหญ่"}</strong><br><address>{text(settings.get('business_address'))}</address></p></div><div><h3>{quick_links}</h3><nav class="footer-quick-links"><a href="{prefix}{base}products/index.html">{products}</a><a href="{prefix}{base}blog/index.html">{blog}</a><a href="{prefix}{base}services/index.html">{services}</a></nav></div><div><h3>{contact}</h3><p><a href="tel:+66877070280">{text(settings.get('phone_display', '087 707 0280'))}</a><br><a href="mailto:{text(settings.get('contact_email'))}">{text(settings.get('contact_email'))}</a></p><div class="social-links" aria-label="{social_label}"><a href="https://www.facebook.com/hkssurfaces" target="_blank" rel="noopener noreferrer" aria-label="Facebook"><i class="bi bi-facebook"></i></a><a href="#" aria-label="YouTube"><i class="bi bi-youtube"></i></a><a href="https://www.instagram.com/hkssurfaces" target="_blank" rel="noopener noreferrer" aria-label="Instagram"><i class="bi bi-instagram"></i></a><a href="https://line.me/ti/p/Rn_AsnrLLf" target="_blank" rel="noopener noreferrer" aria-label="LINE"><i class="bi bi-line"></i></a><a href="#" aria-label="X"><i class="bi bi-twitter-x"></i></a><a href="https://www.threads.com/@hkssurfaces" target="_blank" rel="noopener noreferrer" aria-label="Threads"><i class="bi bi-threads"></i></a></div></div></div><p class="privacy-footer-link"><a href="{prefix}{'en/' if language == 'en' else ''}privacy-policy/index.html">{privacy}</a></p></footer>'''
