...
function projectBody(p){
  ...
  ${editorField("Image path","image",p.image)}</div>${editorField("Thai description","desc_th",p.desc_th,"textarea")}${editorField("English description","desc_en",p.desc_en,"textarea")}`;
}
function blogBody(b){
...
function renderProjects(){
  $("#projectList").innerHTML=state.projects.map((p,i)=>itemShell(p.title_th||p.title_en,p.location_th||p.category,i,"projects",projectBody(p))).join("");
  bindItems();
}
...
$("#addProject").addEventListener("click",()=>{state.projects.push({slug:"new-project",category:"project",title_th:"โครงการใหม่",title_en:"New Project",location_th:"ประเทศไทย",location_en:"Thailand",desc_th:"",desc_en:"",image:"images/your-project.webp"});persist();renderProjects()});
